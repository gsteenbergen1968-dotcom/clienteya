...
function getSuccessTone(successRate: number) {
  if (successRate >= 70) return "border-emerald-200 bg-emerald-50 text-emerald-800";
  if (successRate >= 45) return "border-amber-200 bg-amber-50 text-amber-800";
...
function getPatternLabel(patternId: string | null | undefined) {
  ...
  if (patternId === "hot-client") return "Cliente caliente";
  if (patternId === "converted-client") return "Cliente convertido";
...
export default function WhatsAppLearningCenter({
  ...
  return (
    <section className="overflow-hidden rounded-[34px] border border-slate-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
      <div className="bg-gradient-to-br from-white via-sky-50/60 to-emerald-50 p-5 sm:p-7">
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <span className="inline-flex rounded-full border border-sky-200 bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-sky-700 shadow-sm">
                V18.2 Aprendizaje Comercial
              </span>
