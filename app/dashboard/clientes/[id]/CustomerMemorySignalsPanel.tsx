...
type CustomerMemorySignalsPanelProps = {
  cliente: CommercialMemoryClient;
};

export default function CustomerMemorySignalsPanel({
  cliente,
}: CustomerMemorySignalsPanelProps) {
  const memory = buildCommercialMemorySignals([cliente]);
  const signal = memory.signals[0];

  if (!signal) return null;

  return (
    <section className="rounded-[34px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-6">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">
            ClienteYA resume el patrón comercial más importante de este cliente.
          </p>
        </div>
