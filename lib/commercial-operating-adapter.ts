import {
  ...
  type CommercialOperatingClient,
...
export type CommercialAutomationReminder = {
  cliente: CommercialOperatingClient;
...
export function getCommercialAdapterPriority(
  ...
  if (result.urgency === "critical") return "urgent";
  if (result.urgency === "high") return "high";
  if (result.urgency === "medium") return "medium";
...
export function getCommercialPriorityWeight(
  ...
  if (priority === "today") return 5;
  if (priority === "tomorrow") return 4;
  if (priority === "upcoming") return 3;
...
export function getCommercialAdapterScore(
  ...
  const urgencyScore = getCommercialUrgencyWeight(result.urgency) * 12;
  const priorityScore = getCommercialPriorityWeight(result.priority) * 10;
  ...
  return clamp(Math.round(blendedScore));
}

function buildTitle(result: CommercialOperatingResult) {
  if (result.priority === "today") {
    return `${result.nextActionLabel}: ${result.client.nombre || "Cliente"}`;
  }

  if (result.priority === "tomorrow") {
    return `Preparar mañana: ${result.client.nombre || "Cliente"}`;
  }

  if (result.priority === "upcoming") {
    return `Próximo seguimiento: ${result.client.nombre || "Cliente"}`;
  }

  if (result.priority === "closed") {
    return `Cerrado: ${result.client.nombre || "Cliente"}`;
  }

  return `Monitorear: ${result.client.nombre || "Cliente"}`;
}

function toAutomationReminder(
  result: CommercialOperatingResult,
): CommercialAutomationReminder {
  const priority = getCommercialAdapterPriority(result);

  return {
    cliente: result.client,
    priority,
    score: getCommercialAdapterScore(result),
...
export function buildCommercialAutomationQueue(
  ...
  return buildCommercialOperatingSystem(clients)
    .filter((result) => result.shouldAppearInAutomations)
    .map(toAutomationReminder);
}
...
export function buildCommercialDashboardActions(
  ...
  return buildCommercialOperatingSystem(clients)
    .filter((result) => result.shouldAppearToday)
...
export function buildCommercialCalendarActions(
  ...
  return buildCommercialOperatingSystem(clients)
    .filter((result) => result.shouldAppearInCalendar)
    .map(toCalendarAction);
}
...
export function buildCommercialCockpitSignals(
  ...
  return buildCommercialOperatingSystem(clients)
    .filter((result) => result.shouldAppearInCockpit)
    .map(toCockpitSignal);
}
...
export function buildCommercialOperatingOverview(
  ...
  const operatingResults = buildCommercialOperatingSystem(clients);

  const automationQueue = operatingResults
    .filter((result) => result.shouldAppearInAutomations)
    .map(toAutomationReminder);

  const dashboardActions = operatingResults
    .filter((result) => result.shouldAppearToday)
    .map(toDashboardAction);

  const calendarActions = operatingResults
    .filter((result) => result.shouldAppearInCalendar)
    .map(toCalendarAction);

  const cockpitSignals = operatingResults
    .filter((result) => result.shouldAppearInCockpit)
    .map(toCockpitSignal);
  ...
  return {
    ...
    counts: {
      total: operatingResults.length,
      today: dashboardActions.length,
      calendar: calendarActions.length,
      automations: automationQueue.length,
      cockpit: cockpitSignals.length,
      urgent: automationQueue.filter(
        (item) => item.priority === "urgent",
      ).length,
      high: automationQueue.filter((item) => item.priority === "high")
        .length,
      medium: automationQueue.filter(
        (item) => item.priority === "medium",
      ).length,
      low: automationQueue.filter((item) => item.priority === "low")
        .length,
    },
  };
}